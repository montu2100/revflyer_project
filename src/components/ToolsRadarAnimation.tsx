import React from 'react';

const ToolsRadarAnimation: React.FC = () => {
  return (
    <div className="relative w-[20rem] h-[20rem] sm:w-[24rem] sm:h-[24rem] md:w-[28rem] md:h-[28rem] lg:w-[32rem] lg:h-[32rem] xl:w-[36rem] xl:h-[36rem]">
      <style jsx>{`
        .radar-container {
          position: relative;
          width: 100%;
          height: 100%;
          border-radius: 50%;
          background: #ffffff;
          border: 3px solid #063D3A;
          box-shadow: 
            0 0 0 1px #0f766e,
            0 0 0 5px rgba(15, 118, 110, 0.1);
        }

        .center-logo {
          position: absolute;
          top: 50%;
          left: 50%;
          transform: translate(-50%, -50%);
          width: min(120px, 30%);
          height: min(120px, 30%);
          background: #ffffff;
          border: 2px solid #063D3A;
          border-radius: 50%;
          display: flex;
          align-items: center;
          justify-content: center;
          z-index: 10;
          box-shadow: 0 2px 8px rgba(6, 61, 58, 0.2);
        }

        .radar-icon {
          width: min(80px, 80%);
          height: min(80px, 80%);
          position: relative;
          background: #ffffff;
          border-radius: 50%;
          border: 2px solid #063D3A;
        }

        .radar-icon::before {
          content: '';
          position: absolute;
          top: 50%;
          left: 0;
          right: 0;
          height: 1px;
          background: #0f766e;
          transform: translateY(-50%);
        }

        .radar-icon::after {
          content: '';
          position: absolute;
          top: 0;
          bottom: 0;
          left: 50%;
          width: 1px;
          background: #0f766e;
          transform: translateX(-50%);
        }

        .radar-grid {
          position: absolute;
          top: 50%;
          left: 50%;
          transform: translate(-50%, -50%);
          z-index: 1;
        }

        .radar-circle {
          position: absolute;
          border: 1px solid #3b82f6;
          border-radius: 50%;
          opacity: 0.4;
        }

        .radar-circle-1 {
          width: 35%;
          height: 35%;
          margin: -17.5% 0 0 -17.5%;
        }

        .radar-circle-2 {
          width: 55%;
          height: 55%;
          margin: -27.5% 0 0 -27.5%;
        }

        .radar-circle-3 {
          width: 75%;
          height: 75%;
          margin: -37.5% 0 0 -37.5%;
        }

        .radar-circle-4 {
          width: 95%;
          height: 95%;
          margin: -47.5% 0 0 -47.5%;
        }

        .radar-sweep {
          position: absolute;
          top: 50%;
          left: 50%;
          width: 2px;
          height: min(40px, 10%);
          background: linear-gradient(to top, #1e40af, transparent);
          transform-origin: bottom center;
          transform: translate(-50%, -100%);
          animation: radar-sweep 3s linear infinite;
          z-index: 2;
        }

        @keyframes radar-sweep {
          0% { transform: translate(-50%, -100%) rotate(0deg); opacity: 1; }
          50% { opacity: 0.7; }
          100% { transform: translate(-50%, -100%) rotate(360deg); opacity: 1; }
        }

        .radar-annotations {
          position: absolute;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          z-index: 3;
        }

        .annotation {
          position: absolute;
          font-size: clamp(8px, 2.5vw, 12px);
          color: #1e40af;
          font-weight: bold;
          text-transform: uppercase;
          letter-spacing: 0.5px;
        }

        .annotation-n { top: 2%; left: 50%; transform: translateX(-50%); }
        .annotation-e { right: 3%; top: 50%; transform: translateY(-50%); }
        .annotation-s { bottom: 2%; left: 50%; transform: translateX(-50%); }
        .annotation-w { left: 3%; top: 50%; transform: translateY(-50%); }

        .orbit {
          position: absolute;
          top: 50%;
          left: 50%;
          border-radius: 50%;
          border: 1px dashed rgba(6, 61, 58, 0.3);
        }

        .orbit-1 {
          width: 70%;
          height: 70%;
          margin: -35% 0 0 -35%;
          animation: rotate 25s linear infinite;
        }

        .orbit-2 {
          width: 85%;
          height: 85%;
          margin: -42.5% 0 0 -42.5%;
          animation: rotate 31.25s linear infinite reverse;
        }

        @keyframes rotate {
          from { transform: rotate(0deg); }
          to { transform: rotate(360deg); }
        }

        .logo {
          position: absolute;
          width: clamp(40px, 12vw, 50px);
          height: clamp(40px, 12vw, 50px);
          border-radius: 8px;
          display: flex;
          align-items: center;
          justify-content: center;
          font-weight: 500;
          font-size: clamp(10px, 2.5vw, 12px);
          color: #063D3A;
          background: #ffffff;
          border: 2px solid #0f766e;
          box-shadow: 0 2px 6px rgba(6, 61, 58, 0.2);
          transition: transform 0.3s ease;
          padding: clamp(3px, 1vw, 5px);
        }

        .logo svg {
          width: 100%;
          height: auto;
          max-width: 90%;
          max-height: 70%;
        }

        .logo:hover {
          transform: scale(1.15);
          z-index: 5;
          border-color: #063D3A;
          box-shadow: 0 4px 12px rgba(6, 61, 58, 0.3);
        }

        .logo-1 { top: -6%; left: calc(50% - 6%); }
        .logo-2 { top: calc(13% - 6%); right: calc(13% - 6%); }
        .logo-3 { top: calc(50% - 6%); right: -6%; }
        .logo-4 { bottom: calc(13% - 6%); right: calc(13% - 6%); }
        .logo-5 { bottom: -6%; left: calc(50% - 6%); }
        .logo-6 { top: calc(50% - 6%); left: -6%; }

        .logo-7 { top: calc(15% - 6%); left: calc(50% - 6%); }
        .logo-8 { top: calc(50% - 6%); right: calc(15% - 6%); }
        .logo-9 { bottom: calc(15% - 6%); left: calc(50% - 6%); }
        .logo-10 { top: calc(50% - 6%); left: calc(15% - 6%); }

        .orbit-1 .logo { 
          animation: none;
          transform: rotate(0deg);
        }
        .orbit-2 .logo { 
          animation: none;
          transform: rotate(0deg);
        }

        /* Mobile optimizations */
        @media (max-width: 640px) {
          .radar-container {
            border-width: 2px;
          }
          
          .center-logo {
            width: min(100px, 28%);
            height: min(100px, 28%);
          }
          
          .radar-icon {
            width: min(70px, 75%);
            height: min(70px, 75%);
          }
          
          .logo {
            width: clamp(35px, 15vw, 45px);
            height: clamp(35px, 15vw, 45px);
            font-size: clamp(8px, 3vw, 10px);
          }
          
          .annotation {
            font-size: clamp(7px, 3vw, 10px);
          }
        }
      `}</style>

      <div className="radar-container">
        {/* Radar grid background */}
        <div className="radar-grid">
          <div className="radar-circle radar-circle-1"></div>
          <div className="radar-circle radar-circle-2"></div>
          <div className="radar-circle radar-circle-3"></div>
          <div className="radar-circle radar-circle-4"></div>
        </div>

        {/* Radar annotations */}
        <div className="radar-annotations">
          <div className="annotation annotation-n">N</div>
          <div className="annotation annotation-e">E</div>
          <div className="annotation annotation-s">S</div>
          <div className="annotation annotation-w">W</div>
        </div>

        {/* Center radar logo */}
        <div className="center-logo">
          <div className="radar-icon">
            <div className="radar-sweep"></div>
          </div>
        </div>

        {/* Orbit 1 - Inner circle (6 logos) */}
        <div className="orbit orbit-1">
          <div className="logo logo-1">
            <svg viewBox="0 0 241.4 57" xmlns="http://www.w3.org/2000/svg">
              <style>{`.st0{fill:#FFC127;}`}</style>
              <g>
                <path d="M200.6,13.3c2.4,0,4.3-1.9,4.3-4.1S203,5,200.6,5c-2.4,0-4.3,1.9-4.3,4.1S198.2,13.3,200.6,13.3z"/>
                <path d="M107.3,46.7c-9.5,0-16.4-6.4-16.4-15.2c0-8.8,6.9-15.1,16.4-15.1c4.6,0,8.8,1.5,11.8,4.2c3,2.7,4.6,6.6,4.6,10.9c0,4.3-1.7,8.1-4.6,10.9C116.1,45.3,111.9,46.7,107.3,46.7z M107.3,23.1c-4.9,0-8.4,3.5-8.4,8.5s3.4,8.5,8.4,8.5c5,0,8.4-3.4,8.4-8.5C115.7,26.5,112.3,23.1,107.3,23.1L107.3,23.1z"/>
                <path d="M135.2,5.6h-7.9v40.7h7.9V5.6z"/>
                <path d="M147.2,5.6h-7.9v40.7h7.9V5.6z"/>
                <path d="M167.1,46.7c-9.5,0-16.4-6.4-16.4-15.2c0-8.8,6.9-15.1,16.4-15.1c4.6,0,8.8,1.5,11.8,4.2c3,2.7,4.6,6.6,4.6,10.9c0,4.3-1.7,8.1-4.6,10.9C175.9,45.3,171.7,46.7,167.1,46.7z M167.1,23.1c-4.9,0-8.4,3.5-8.4,8.5s3.4,8.5,8.4,8.5c5,0,8.4-3.4,8.4-8.5C175.5,26.5,172.2,23.1,167.1,23.1L167.1,23.1z"/>
                <path d="M204.6,16.5h-7.9v29.9h7.9V16.5z"/>
                <path d="M225,46.7c-9.5,0-16.4-6.4-16.4-15.2c0-8.8,6.9-15.1,16.4-15.1c4.6,0,8.8,1.5,11.8,4.2c3,2.7,4.6,6.6,4.6,10.9c0,4.3-1.7,8.1-4.6,10.9C233.8,45.3,229.6,46.7,225,46.7z M225,23.1c-4.9,0-8.4,3.5-8.4,8.5s3.4,8.5,8.4,8.5c5,0,8.4-3.4,8.4-8.5C233.4,26.5,230,23.1,225,23.1L225,23.1z"/>
                <path d="M55.4,57V16.3h7.9v2.2c2.6-1.3,5.5-2,8.4-1.9c4.6,0,8.8,1.5,11.8,4.2c3,2.7,4.6,6.6,4.6,10.9c0,4.3-1.7,8.1-4.6,10.9c-3,2.8-7.2,4.3-11.8,4.3c-2.9,0-5.8-0.6-8.4-2V57H55.4z M71.7,23.1c-4.9,0-8.4,3.5-8.4,8.5c0,5,3.4,8.5,8.4,8.5c5,0,8.4-3.4,8.4-8.5C80.2,26.5,76.8,23.1,71.7,23.1z"/>
                <path d="M189,46.4c-0.7,0-1.3-0.2-1.9-0.6c-0.6-0.4-1-0.9-1.3-1.5s-0.3-1.3-0.2-1.9c0.1-0.6,0.5-1.2,0.9-1.7c0.5-0.5,1.1-0.8,1.8-0.9c0.7-0.1,1.4-0.1,2,0.2c0.6,0.2,1.2,0.7,1.5,1.2c0.4,0.5,0.6,1.2,0.6,1.8c0,0.9-0.4,1.7-1,2.3C190.8,46,189.9,46.4,189,46.4z"/>
                <path className="st0" d="M44.8,9h-7.5l3.9,6.5L44.8,9z"/>
                <path d="M49,34.9L44,26.4L28,0L0,46.4h15c2,0,4-0.5,5.7-1.5c1.7-1,3.2-2.3,4.2-4l7.7-12.8l-4.3-7.1L19,36.5c-0.5,0.9-1.3,1.6-2.2,2.1c-0.9,0.5-1.9,0.8-2.9,0.8h-1.1L28,14.1l19.4,32.2h5.5v-5.1L49,34.9L49,34.9z"/>
              </g>
            </svg>
          </div>
          <div className="logo logo-2">
            <svg viewBox="0 0 131 39" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g clipPath="url(#clip0)">
                <path d="M81.1117 11.7172L77.8545 14.1619L77.7327 13.9805C76.2119 11.7526 73.728 10.3513 70.7427 10.3513C65.6296 10.3513 61.8734 14.0724 61.8734 19.4986C61.8734 24.9248 65.6273 28.6459 70.7427 28.6459C73.7116 28.6459 76.1603 27.2988 77.7163 25.0332L77.8545 24.8353L81.1257 27.2894L81.0062 27.459C78.8785 30.6031 75.1246 32.546 70.7427 32.546C63.0544 32.546 57.6367 27.0115 57.6367 19.4986C57.6367 11.9857 63.0544 6.45117 70.7427 6.45117C75.1949 6.45117 79.1457 8.44597 81.1117 11.7172Z" fill="black"/>
                <path d="M86.6231 32.078H82.8691V6.20215H86.6231V32.078Z" fill="black"/>
                <path d="M108.544 32.0773H104.806V30.0119C103.625 31.538 101.856 32.5272 99.4239 32.5272C94.632 32.5272 90.8594 28.7355 90.8594 23.4718C90.8594 18.2058 94.632 14.4165 99.4239 14.4165C101.856 14.4165 103.625 15.4221 104.806 16.9506V14.8663H108.544V32.0773ZM94.7046 23.4695C94.7046 26.4887 96.6542 28.9851 99.8715 28.9851C102.965 28.9851 105.038 26.6135 105.038 23.4695C105.038 20.3254 102.965 17.9539 99.8715 17.9539C96.6542 17.9539 94.7046 20.4503 94.7046 23.4695Z" fill="black"/>
                <path d="M124.333 14.8997L123.438 18.6184C122.651 18.2039 121.507 17.9896 120.596 17.9896C118.218 17.9896 116.519 19.6782 116.519 22.6079V32.0777H112.781V14.8644H116.481V16.805C117.608 15.1705 119.359 14.4146 121.451 14.4146C122.599 14.4146 123.473 14.5582 124.333 14.8997Z" fill="black"/>
                <path d="M131 8.9148C131 10.4056 129.838 11.5384 128.355 11.5384C126.871 11.5384 125.744 10.4056 125.744 8.9148C125.744 7.424 126.871 6.25586 128.355 6.25586C129.838 6.25586 131 7.424 131 8.9148ZM130.248 32.0775H126.494V14.8639H130.248V32.0775Z" fill="black"/>
                <path d="M44.5468 7.40039L30.5879 11.1921C31.1526 13.811 31.4502 16.5877 31.4502 19.501C31.4502 22.4143 31.1503 25.1887 30.5879 27.8099L44.5468 31.6017C45.4396 27.8193 45.9082 23.6743 45.9082 19.5034C45.9082 15.3325 45.4396 11.1827 44.5468 7.40039Z" fill="#00D7B8"/>
                <path d="M9.20182 3.48193L0 19.4988L14.5631 8.57125C13.1665 6.47759 11.3482 4.75367 9.20182 3.48193Z" fill="#5F3AD7"/>
                <path d="M9.20182 35.5187L0 19.4995L14.5631 30.427C13.1665 32.5231 11.3482 34.247 9.20182 35.5187Z" fill="#5F3AD7"/>
                <path d="M25.9895 0L0 19.5006L30.5893 11.1917C29.6942 7.03482 28.1265 3.2713 25.9895 0Z" fill="#0280FF"/>
                <path d="M25.9895 38.9996L0 19.499L30.5893 27.808C29.6942 31.9672 28.1265 35.7307 25.9895 38.9996Z" fill="#0280FF"/>
                <path d="M31.454 19.4989C31.454 16.5856 31.1541 13.8112 30.5917 11.1899L0 19.4989L30.5893 27.8079C31.1564 25.1842 31.454 22.4169 31.454 19.4989Z" fill="white"/>
              </g>
              <defs>
                <clipPath id="clip0">
                  <rect width="131" height="39" fill="white"/>
                </clipPath>
              </defs>
            </svg>
          </div>
          <div className="logo logo-3">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 184 40" fill="currentColor">
              <path shapeRendering="optimizeQuality" fill="#D97757" d="m7.75 26.27 7.77-4.36.13-.38-.13-.21h-.38l-1.3-.08-4.44-.12-3.85-.16-3.73-.2-.94-.2L0 19.4l.09-.58.79-.53 1.13.1 2.5.17 3.75.26 2.72.16 4.03.42h.64l.09-.26-.22-.16-.17-.16-3.88-2.63-4.2-2.78-2.2-1.6L3.88 11l-.6-.76-.26-1.66L4.1 7.39l1.45.1.37.1 1.47 1.13 3.14 2.43 4.1 3.02.6.5.24-.17.03-.12-.27-.45L13 9.9l-2.38-4.1-1.06-1.7-.28-1.02c-.1-.42-.17-.77-.17-1.2L10.34.21l.68-.22 1.64.22.69.6 1.02 2.33 1.65 3.67 2.56 4.99.75 1.48.4 1.37.15.42h.26v-.24l.21-2.81.39-3.45.38-4.44.13-1.25.62-1.5L23.1.57l.96.46.79 1.13-.11.73-.47 3.05-.92 4.78-.6 3.2h.35l.4-.4 1.62-2.15 2.72-3.4 1.2-1.35 1.4-1.49.9-.71h1.7l1.25 1.86-.56 1.92-1.75 2.22-1.45 1.88-2.08 2.8-1.3 2.24.12.18.31-.03 4.7-1 2.54-.46 3.03-.52 1.37.64.15.65-.54 1.33-3.24.8-3.8.76-5.66 1.34-.07.05.08.1 2.55.24 1.09.06h2.67l4.97.37 1.3.86.78 1.05-.13.8-2 1.02-2.7-.64-6.3-1.5-2.16-.54h-.3v.18l1.8 1.76 3.3 2.98 4.13 3.84.21.95-.53.75-.56-.08-3.63-2.73-1.4-1.23-3.17-2.67h-.21v.28l.73 1.07 3.86 5.8.2 1.78-.28.58-1 .35-1.1-.2L26 33.14l-2.33-3.57-1.88-3.2-.23.13-1.11 11.95-.52.61-1.2.46-1-.76-.53-1.23.53-2.43.64-3.17.52-2.52.47-3.13.28-1.04-.02-.07-.23.03-2.36 3.24-3.59 4.85-2.84 3.04-.68.27-1.18-.61.11-1.09.66-.97 3.93-5 2.37-3.1 1.53-1.79-.01-.26h-.09L6.8 30.56l-1.86.24-.8-.75.1-1.23.38-.4 3.14-2.16Z"/>
              <path shapeRendering="optimizeQuality" d="M64.48 33.54c-5.02 0-8.45-2.8-10.07-7.11a19.19 19.19 0 0 1-1.23-7.03c0-7.23 3.24-12.25 10.4-12.25 4.81 0 7.78 2.1 9.47 7.11h2.06l-.28-6.91c-2.88-1.86-6.48-2.8-10.86-2.8-6.17 0-11.42 2.76-14.34 7.74a16.77 16.77 0 0 0-2.22 8.65c0 5.53 2.61 10.43 7.51 13.15a17.51 17.51 0 0 0 8.73 2.06c4.78 0 8.57-.91 11.93-2.5l.87-7.62h-2.1c-1.26 3.48-2.76 5.57-5.25 6.68-1.22.55-2.76.83-4.62.83ZM86.13 7.15l.2-3.4h-1.42l-6.32 1.9v1.03l2.8 1.3v23.78c0 1.62-.83 1.98-3 2.25v1.74h10.75v-1.74c-2.18-.27-3-.63-3-2.25V7.16Zm42.75 29h.83l7.27-1.38v-1.78l-1.02-.08c-1.7-.16-2.14-.51-2.14-1.9V18.33l.2-4.07h-1.15l-6.87.99v1.74l.67.12c1.86.27 2.41.79 2.41 2.09v11.3c-1.78 1.38-3.48 2.25-5.5 2.25-2.24 0-3.63-1.14-3.63-3.8V18.34l.2-4.07h-1.18l-6.88.99v1.74l.71.12c1.86.27 2.41.79 2.41 2.09v10.43c0 4.42 2.5 6.52 6.48 6.52 3.04 0 5.53-1.62 7.4-3.87l-.2 3.87ZM108.9 22.08c0-5.65-3-7.82-8.42-7.82-4.78 0-8.25 1.98-8.25 5.26 0 .98.35 1.73 1.06 2.25l3.64-.48c-.16-1.1-.24-1.77-.24-2.05 0-1.86.99-2.8 3-2.8 2.97 0 4.47 2.09 4.47 5.45v1.1l-7.5 2.25c-2.5.68-3.92 1.27-4.87 2.65a5 5 0 0 0-.7 2.8c0 3.2 2.2 5.46 5.96 5.46 2.72 0 5.13-1.23 7.23-3.56.75 2.33 1.9 3.56 3.95 3.56 1.66 0 3.16-.67 4.5-1.98l-.4-1.38c-.58.16-1.14.24-1.73.24-1.15 0-1.7-.91-1.7-2.69v-8.26Zm-9.6 10.87c-2.05 0-3.32-1.19-3.32-3.28 0-1.42.67-2.25 2.1-2.73l6.08-1.93v5.84c-1.94 1.47-3.08 2.1-4.86 2.1Zm63.3 1.82v-1.78l-1.03-.08c-1.7-.16-2.13-.51-2.13-1.9V7.15l.2-3.4h-1.43l-6.32 1.9v1.03l2.8 1.3v7.82a8.83 8.83 0 0 0-5.37-1.54c-6.28 0-11.18 4.78-11.18 11.93 0 5.89 3.52 9.96 9.32 9.96 3 0 5.61-1.46 7.23-3.72l-.2 3.72h.84l7.27-1.38Zm-13.16-18.14c3 0 5.25 1.74 5.25 4.94v9a7.2 7.2 0 0 1-5.21 2.1c-4.3 0-6.48-3.4-6.48-7.94 0-5.1 2.49-8.1 6.44-8.1Zm28.53 4.5c-.56-2.64-2.18-4.14-4.43-4.14-3.36 0-5.69 2.53-5.69 6.16 0 5.37 2.84 8.85 7.43 8.85a8.6 8.6 0 0 0 7.39-4.35l1.34.36c-.6 4.66-4.82 8.14-10 8.14-6.08 0-10.27-4.5-10.27-10.9 0-6.45 4.55-10.99 10.63-10.99 4.54 0 7.74 2.73 8.77 7.47l-15.84 4.86v-2.14l10.67-3.31Z"/>
            </svg>
          </div>
          <div className="logo logo-4">
            <svg fill="none" height="40" viewBox="0 0 143 40" width="143" xmlns="http://www.w3.org/2000/svg">
              <path clipRule="evenodd" d="M78.9736 8C78.9736 12.4183 75.3919 16 70.9736 16C67.246 16 64.1138 13.4505 63.2257 10H52.0564C50.101 10 48.4323 11.4137 48.1108 13.3424L47.782 15.3152C47.4698 17.1883 46.5223 18.8185 45.1824 20C46.5223 21.1815 47.4698 22.8117 47.782 24.6848L48.1108 26.6576C48.4323 28.5863 50.101 30 52.0564 30H55.2257C56.1138 26.5495 59.246 24 62.9736 24C67.3919 24 70.9736 27.5817 70.9736 32C70.9736 36.4183 67.3919 40 62.9736 40C59.246 40 56.1138 37.4505 55.2257 34H52.0564C48.1457 34 44.8082 31.1727 44.1653 27.3152L43.8364 25.3424C43.515 23.4137 41.8462 22 39.8909 22H36.628C35.6294 25.2801 32.5802 27.6667 28.9736 27.6667C25.367 27.6667 22.3179 25.2801 21.3193 22H16.628C15.6294 25.2801 12.5802 27.6667 8.97363 27.6667C4.55536 27.6667 0.973633 24.0849 0.973633 19.6667C0.973633 15.2484 4.55536 11.6667 8.97363 11.6667C12.8204 11.6667 16.033 14.3817 16.7998 18H21.1475C21.9143 14.3817 25.1269 11.6667 28.9736 11.6667C32.8204 11.6667 36.033 14.3817 36.7998 18H39.8909C41.8462 18 43.515 16.5863 43.8364 14.6576L44.1653 12.6848C44.8082 8.8273 48.1457 6 52.0564 6L63.2257 6C64.1138 2.54955 67.246 0 70.9736 0C75.3919 0 78.9736 3.58172 78.9736 8ZM74.9736 8C74.9736 10.2091 73.1828 12 70.9736 12C68.7645 12 66.9736 10.2091 66.9736 8C66.9736 5.79086 68.7645 4 70.9736 4C73.1828 4 74.9736 5.79086 74.9736 8ZM8.97363 23.6667C11.1828 23.6667 12.9736 21.8758 12.9736 19.6667C12.9736 17.4575 11.1828 15.6667 8.97363 15.6667C6.76449 15.6667 4.97363 17.4575 4.97363 19.6667C4.97363 21.8758 6.76449 23.6667 8.97363 23.6667ZM28.9736 23.6667C31.1828 23.6667 32.9736 21.8758 32.9736 19.6667C32.9736 17.4575 31.1828 15.6667 28.9736 15.6667C26.7645 15.6667 24.9736 17.4575 24.9736 19.6667C24.9736 21.8758 26.7645 23.6667 28.9736 23.6667ZM62.9736 36C65.1828 36 66.9736 34.2091 66.9736 32C66.9736 29.7909 65.1828 28 62.9736 28C60.7645 28 58.9736 29.7909 58.9736 32C58.9736 34.2091 60.7645 36 62.9736 36Z" fill="#EA4B71" fillRule="evenodd"/>
              <path d="M86.9785 30.001H91.1554V21.1578C91.1554 18.2536 92.9175 16.981 94.908 16.981C96.8659 16.981 98.3996 18.2862 98.3996 20.962V30.001H102.576V20.1136C102.576 15.8389 100.096 13.3589 96.2133 13.3589C93.7659 13.3589 92.3954 14.3378 91.4164 15.6104H91.1554L90.7964 13.6852H86.9785V30.001Z" fill="#101330"/>
              <path d="M118.563 19.0041V18.8083C119.999 18.0904 121.435 16.8504 121.435 14.4031C121.435 10.8789 118.53 8.75781 114.517 8.75781C110.405 8.75781 107.468 11.0094 107.468 14.4683C107.468 16.8178 108.839 18.0904 110.34 18.8083V19.0041C108.676 19.5915 106.685 21.3536 106.685 24.2904C106.685 27.8473 109.622 30.3273 114.484 30.3273C119.346 30.3273 122.185 27.8473 122.185 24.2904C122.185 21.3536 120.227 19.6241 118.563 19.0041ZM114.484 11.7599C116.116 11.7599 117.323 12.8041 117.323 14.5662C117.323 16.3283 116.083 17.3726 114.484 17.3726C112.885 17.3726 111.547 16.3283 111.547 14.5662C111.547 12.7715 112.82 11.7599 114.484 11.7599ZM114.484 27.1947C112.591 27.1947 111.058 25.9873 111.058 23.9315C111.058 22.0715 112.33 20.6683 114.451 20.6683C116.54 20.6683 117.813 22.0389 117.813 23.9968C117.813 25.9873 116.344 27.1947 114.484 27.1947Z" fill="#101330"/>
              <path d="M126.428 30.001H130.605V21.1578C130.605 18.2536 132.367 16.981 134.358 16.981C136.316 16.981 137.849 18.2862 137.849 20.962V30.001H142.026V20.1136C142.026 15.8389 139.546 13.3589 135.663 13.3589C133.216 13.3589 131.845 14.3378 130.866 15.6104H130.605L130.246 13.6852H126.428V30.001Z" fill="#101330"/>
            </svg>
          </div>
          <div className="logo logo-5">
            <svg xmlns="http://www.w3.org/2000/svg" width="2500" height="1750" viewBox="0.5 0.5 999 699.242">
              <path fill="#00A1E0" d="M416.224 76.763c32.219-33.57 77.074-54.391 126.682-54.391 65.946 0 123.48 36.772 154.12 91.361 26.626-11.896 56.098-18.514 87.106-18.514 118.94 0 215.368 97.268 215.368 217.247 0 119.993-96.428 217.261-215.368 217.261a213.735 213.735 0 0 1-42.422-4.227c-26.981 48.128-78.397 80.646-137.412 80.646-24.705 0-48.072-5.706-68.877-15.853-27.352 64.337-91.077 109.448-165.348 109.448-77.344 0-143.261-48.939-168.563-117.574-11.057 2.348-22.513 3.572-34.268 3.572C75.155 585.74.5 510.317.5 417.262c0-62.359 33.542-116.807 83.378-145.937-10.26-23.608-15.967-49.665-15.967-77.06C67.911 87.25 154.79.5 261.948.5c62.914 0 118.827 29.913 154.276 76.263"/>
              <path fill="#FFF" d="M145.196 363.11c-.626 1.637.228 1.979.427 2.263 1.878 1.366 3.786 2.349 5.707 3.444 10.189 5.407 19.81 6.986 29.871 6.986 20.492 0 33.214-10.9 33.214-28.447v-.341c0-16.224-14.358-22.115-27.835-26.37l-1.75-.569c-10.161-3.302-18.927-6.147-18.927-12.836v-.355c0-5.721 5.123-9.934 13.064-9.934 8.823 0 19.297 2.932 26.042 6.66 0 0 1.978 1.281 2.704-.64.398-1.025 3.814-10.218 4.17-11.214.384-1.082-.299-1.879-.996-2.306-7.699-4.682-18.344-7.884-29.358-7.884l-2.049.014c-18.756 0-31.848 11.328-31.848 27.565v.342c0 17.119 14.444 22.669 27.978 26.54l2.177.669c9.862 3.031 18.358 5.635 18.358 12.58v.342c0 6.347-5.521 11.071-14.43 11.071-3.458 0-14.487-.071-26.398-7.6-1.438-.84-2.277-1.451-3.387-2.12-.583-.37-2.049-1.011-2.689.925l-4.045 11.215zM445.194 363.11c-.626 1.637.228 1.979.427 2.263 1.878 1.366 3.786 2.349 5.706 3.444 10.189 5.407 19.811 6.986 29.871 6.986 20.492 0 33.215-10.9 33.215-28.447v-.341c0-16.224-14.359-22.115-27.836-26.37l-1.75-.569c-10.161-3.302-18.928-6.147-18.928-12.836v-.355c0-5.721 5.123-9.934 13.064-9.934 8.823 0 19.297 2.932 26.043 6.66 0 0 1.978 1.281 2.703-.64.398-1.025 3.814-10.218 4.17-11.214.385-1.082-.299-1.879-.996-2.306-7.699-4.682-18.344-7.884-29.358-7.884l-2.05.014c-18.756 0-31.848 11.328-31.848 27.565v.342c0 17.119 14.444 22.669 27.978 26.54l2.177.669c9.862 3.031 18.373 5.635 18.373 12.58v.342c0 6.347-5.536 11.071-14.445 11.071-3.457 0-14.486-.071-26.397-7.6-1.438-.84-2.291-1.423-3.372-2.12-.371-.242-2.107-.911-2.705.925l-4.042 11.215zM649.995 328.74c0 9.919-1.85 17.731-5.493 23.253-3.601 5.465-9.051 8.126-16.649 8.126-7.613 0-13.035-2.647-16.579-8.126-3.587-5.507-5.407-13.334-5.407-23.253 0-9.904 1.82-17.703 5.407-23.168 3.544-5.407 8.966-8.04 16.579-8.04 7.599 0 13.049 2.633 16.664 8.04 3.629 5.464 5.478 13.263 5.478 23.168m17.106-18.386c-1.68-5.679-4.298-10.688-7.784-14.857-3.487-4.184-7.898-7.542-13.136-9.99-5.223-2.433-11.398-3.671-18.328-3.671-6.945 0-13.121 1.238-18.344 3.671-5.237 2.448-9.648 5.807-13.149 9.99-3.472 4.184-6.091 9.193-7.784 14.857-1.665 5.649-2.505 11.825-2.505 18.386s.84 12.751 2.505 18.386c1.693 5.664 4.298 10.674 7.799 14.857 3.486 4.184 7.912 7.528 13.135 9.904 5.236 2.377 11.398 3.586 18.344 3.586 6.93 0 13.092-1.209 18.328-3.586 5.223-2.376 9.648-5.721 13.136-9.904 3.486-4.17 6.104-9.179 7.784-14.857 1.68-5.649 2.519-11.84 2.519-18.386s-.841-12.737-2.52-18.386M807.568 357.47c-.569-1.665-2.177-1.039-2.177-1.039-2.49.954-5.138 1.836-7.955 2.277-2.861.44-6.006.669-9.379.669-8.281 0-14.856-2.462-19.566-7.329-4.725-4.867-7.372-12.736-7.344-23.381.029-9.691 2.362-16.978 6.561-22.527 4.17-5.521 10.517-8.354 18.984-8.354 7.059 0 12.438.811 18.072 2.59 0 0 1.352.583 1.992-1.181 1.494-4.156 2.604-7.13 4.198-11.698.456-1.295-.654-1.85-1.053-2.007-2.22-.868-7.457-2.276-11.413-2.874-3.7-.569-8.026-.868-12.836-.868-7.188 0-13.591 1.224-19.069 3.672-5.465 2.433-10.104 5.791-13.775 9.976-3.672 4.184-6.461 9.192-8.325 14.856-1.85 5.649-2.789 11.854-2.789 18.415 0 14.188 3.828 25.657 11.385 34.054 7.57 8.425 18.941 12.708 33.77 12.708 8.766 0 17.76-1.778 24.221-4.326 0 0 1.238-.598.697-2.034l-4.199-11.599zM837.497 319.238c.811-5.507 2.334-10.09 4.682-13.661 3.544-5.422 8.951-8.396 16.551-8.396s12.623 2.988 16.223 8.396c2.391 3.571 3.43 8.354 3.843 13.661h-41.299zm57.592-12.111c-1.451-5.479-5.052-11.015-7.414-13.548-3.729-4.013-7.371-6.816-10.986-8.382-4.725-2.021-10.389-3.358-16.593-3.358-7.229 0-13.79 1.21-19.112 3.714-5.336 2.505-9.818 5.921-13.334 10.176-3.516 4.24-6.162 9.292-7.842 15.027-1.693 5.707-2.547 11.926-2.547 18.485 0 6.675.883 12.894 2.633 18.486 1.765 5.636 4.582 10.602 8.396 14.714 3.799 4.142 8.695 7.387 14.558 9.648 5.821 2.249 12.894 3.416 21.019 3.401 16.722-.057 25.53-3.785 29.159-5.792.641-.355 1.253-.981.483-2.774l-3.785-10.603c-.568-1.579-2.177-.996-2.177-.996-4.142 1.537-10.032 4.298-23.766 4.27-8.979-.014-15.64-2.661-19.81-6.803-4.283-4.24-6.375-10.474-6.745-19.268l57.905.057s1.522-.028 1.68-1.509c.057-.624 1.993-11.895-1.722-24.945M373.762 319.238c.825-5.507 2.334-10.09 4.682-13.661 3.543-5.422 8.951-8.396 16.55-8.396s12.623 2.988 16.237 8.396c2.376 3.571 3.415 8.354 3.828 13.661h-41.297zm57.577-12.111c-1.451-5.479-5.037-11.015-7.399-13.548-3.729-4.013-7.372-6.816-10.986-8.382-4.725-2.021-10.388-3.358-16.593-3.358-7.215 0-13.79 1.21-19.112 3.714-5.336 2.505-9.819 5.921-13.334 10.176-3.515 4.24-6.162 9.292-7.841 15.027-1.679 5.707-2.547 11.926-2.547 18.485 0 6.675.882 12.894 2.633 18.486 1.765 5.636 4.583 10.602 8.396 14.714 3.8 4.142 8.695 7.387 14.558 9.648 5.821 2.249 12.893 3.416 21.019 3.401 16.721-.057 25.53-3.785 29.159-5.792.641-.355 1.252-.981.484-2.774l-3.771-10.603c-.584-1.579-2.191-.996-2.191-.996-4.141 1.537-10.019 4.298-23.78 4.27-8.965-.014-15.625-2.661-19.795-6.803-4.284-4.24-6.375-10.474-6.746-19.268l57.905.057s1.522-.028 1.679-1.509c.055-.624 1.99-11.895-1.738-24.945M248.601 357.153c-2.263-1.808-2.576-2.263-3.344-3.43-1.139-1.779-1.722-4.312-1.722-7.528 0-5.095 1.679-8.752 5.166-11.214-.042.015 4.981-4.34 16.792-4.184 8.296.114 15.71 1.338 15.71 1.338v26.327h.014s-7.357 1.579-15.639 2.077c-11.783.712-17.02-3.4-16.977-3.386m23.039-40.686c-2.348-.171-5.394-.271-9.037-.271-4.966 0-9.762.626-14.259 1.836-4.525 1.209-8.595 3.103-12.096 5.606a27.927 27.927 0 0 0-8.396 9.549c-2.049 3.814-3.088 8.311-3.088 13.349 0 5.123.882 9.577 2.647 13.221 1.765 3.657 4.312 6.702 7.556 9.051 3.216 2.348 7.187 4.069 11.797 5.108 4.54 1.039 9.691 1.565 15.327 1.565 5.934 0 11.854-.483 17.589-1.466 5.678-.968 12.651-2.377 14.586-2.817a146.25 146.25 0 0 0 4.056-1.039c1.438-.355 1.324-1.893 1.324-1.893l-.029-52.952c0-11.613-3.102-20.223-9.207-25.559-6.077-5.322-15.028-8.013-26.597-8.013-4.341 0-11.328.599-15.512 1.438 0 0-12.651 2.448-17.86 6.518 0 0-1.138.712-.512 2.306l4.099 11.015c.512 1.423 1.893.939 1.893.939s.441-.171.954-.47c11.143-6.062 25.231-5.877 25.231-5.877 6.262 0 11.072 1.252 14.316 3.742 3.159 2.419 4.767 6.076 4.767 13.789v2.448c-4.981-.711-9.549-1.123-9.549-1.123M738.669 286.631c.44-1.31-.484-1.936-.869-2.078-.981-.384-5.905-1.423-9.705-1.665-7.271-.441-11.312.783-14.928 2.405-3.586 1.622-7.57 4.24-9.791 7.215v-7.044c0-.982-.697-1.765-1.665-1.765h-14.843c-.967 0-1.664.782-1.664 1.765v86.366c0 .968.797 1.765 1.764 1.765h15.213a1.76 1.76 0 0 0 1.75-1.765v-43.147c0-5.792.641-11.569 1.922-15.198 1.252-3.587 2.96-6.461 5.066-8.525 2.12-2.049 4.525-3.486 7.158-4.297 2.689-.826 5.663-1.096 7.77-1.096 3.031 0 6.361.782 6.361.782 1.109.128 1.736-.555 2.105-1.565.997-2.647 3.815-10.574 4.356-12.153"/>
              <path fill="#FFF" d="M595.874 246.603c-1.85-.569-3.529-.954-5.721-1.366-2.221-.398-4.867-.598-7.869-.598-10.475 0-18.729 2.96-24.52 8.794-5.764 5.807-9.678 14.644-11.642 26.271l-.712 3.913h-13.148s-1.594-.057-1.936 1.68l-2.148 12.053c-.157 1.139.342 1.864 1.878 1.864h12.794l-12.979 72.463c-1.011 5.835-2.178 10.631-3.473 14.273-1.267 3.587-2.504 6.276-4.041 8.24-1.48 1.879-2.875 3.273-5.295 4.084-1.992.669-4.297.982-6.816.982-1.395 0-3.258-.229-4.639-.513-1.366-.271-2.092-.569-3.131-1.011 0 0-1.494-.568-2.092.926-.47 1.238-3.885 10.615-4.298 11.769-.398 1.152.171 2.049.896 2.319 1.708.598 2.974.996 5.294 1.551 3.217.755 5.934.797 8.481.797 5.322 0 10.189-.754 14.217-2.205 4.042-1.466 7.57-4.014 10.701-7.457 3.373-3.729 5.493-7.628 7.515-12.964 2.006-5.266 3.729-11.812 5.094-19.439l13.05-73.815h19.069s1.607.057 1.936-1.693l2.162-12.039c.143-1.152-.341-1.864-1.893-1.864h-18.514c.1-.412.939-6.931 3.06-13.063.911-2.604 2.618-4.725 4.056-6.177 1.424-1.423 3.06-2.433 4.854-3.017 1.835-.598 3.928-.882 6.219-.882 1.736 0 3.457.199 4.752.469 1.793.385 2.49.584 2.961.727 1.893.569 2.148.014 2.519-.896l4.426-12.153c.455-1.312-.669-1.867-1.067-2.023M337.194 371.834c0 .968-.697 1.751-1.665 1.751h-15.355c-.968 0-1.651-.783-1.651-1.751v-123.58c0-.967.683-1.75 1.651-1.75h15.355c.968 0 1.665.783 1.665 1.75v123.58z"/>
            </svg>
          </div>
          <div className="logo logo-6">
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 218.5 50">
              <style type="text/css">
                {`.st0{fill:#1F1E1F;} .st1{fill-rule:evenodd;clip-rule:evenodd;fill:url(#Shape_1_);}`}
              </style>
              <defs>
                <linearGradient id="Shape_1_" gradientUnits="userSpaceOnUse" x1="138.6377" y1="210.8306" x2="140.8677" y2="208.5125" gradientTransform="matrix(18.5523 0 0 -19.2438 -2566.748 4062.1538)">
                  <stop offset="0" style={{stopColor:'#40E0CF'}}/>
                  <stop offset="1" style={{stopColor:'#36CDCF'}}/>
                </linearGradient>
              </defs>
              <g>
                <g transform="translate(-134.000000, -280.000000)">
                  <g transform="translate(134.000000, 280.000000)">
                    <g transform="translate(0.000000, 0.000000)">
                      <g transform="translate(32.000000, 3.000000)">
                        <path className="st0" d="M33.9,36.6V25.9c0-4.2,1.9-6.6,6-6.6c0.6,0,1.2,0,1.8,0.1V14c-0.2,0-0.8-0.1-1.4-0.1c-2.9,0-5.4,1.4-6.5,3.9v-3.6h-5.2v22.4H33.9z M46.9,9.7c1.9,0,3.5-1.5,3.5-3.5s-1.6-3.5-3.5-3.5c-1.9,0-3.5,1.6-3.5,3.5S45,9.7,46.9,9.7z M49.6,36.6V14.1h-5.3v22.4H49.6z M59.9,36.6v-13c0-2.9,1.6-5.3,4.7-5.3c3.4,0,4.6,2.2,4.6,5v13.3h5.3V22.4c0-4.9-2.6-8.9-8.1-8.9c-2.5,0-5.2,1.1-6.6,3.6v-3h-5.2v22.4H59.9z M88.2,46c8.4,0,11.5-5.6,11.5-11.6V14.1h-5.2V17c-1-1.8-3.1-3.3-6.6-3.3c-6.2,0-10.4,4.9-10.4,10.9c0,6.3,4.4,10.9,10.4,10.9c3.3,0,5.5-1.5,6.5-3.3v2.4c0,4.6-2.1,6.7-6.4,6.7c-3.1,0-5.3-2.1-5.6-4.8l-4.9,1.3C78.1,42.4,82.3,46,88.2,46z M88.8,31c-3.5,0-5.9-2.4-5.9-6.4c0-3.8,2.4-6.3,5.9-6.3c3.3,0,5.8,2.5,5.8,6.3C94.6,28.5,92.2,31,88.8,31z M114.2,37.3c6.7,0,11.7-5,11.7-11.9c0-6.9-4.9-11.9-11.7-11.9c-6.7,0-11.6,5-11.6,11.9C102.6,32.3,107.5,37.3,114.2,37.3z M114.2,32.5c-3.3,0-6.3-2.5-6.3-7.1s3-7.1,6.3-7.1c3.3,0,6.3,2.4,6.3,7.1C120.5,30,117.5,32.5,114.2,32.5z M139.9,36.6l8.8-22.4h-5.6l-5.8,16l-5.9-16h-5.9l9.1,22.4H139.9z M159.7,37.3c5.4,0,8.9-3.2,10.1-7.1l-4.5-1.4c-0.7,2.2-2.4,3.9-5.6,3.9c-3.3,0-6-2.4-6.1-5.9H170c0-0.1,0.1-0.8,0.1-1.7c0-7.1-4.1-11.7-11-11.7c-5.7,0-10.9,4.7-10.9,11.8C148.3,32.9,153.6,37.3,159.7,37.3z M164.8,22.9h-11c0.2-2.4,2.1-5,5.5-5C162.9,17.9,164.7,20.3,164.8,22.9z M178.7,36.6V25.9c0-4.2,1.9-6.6,6-6.6c0.6,0,1.2,0,1.8,0.1V14c-0.2,0-0.8-0.1-1.4-0.1c-2.9,0-5.4,1.4-6.5,3.9v-3.6h-5.2v22.4H178.7z"/>
                      </g>
                      <g transform="translate(0.000000, 0.000000)">
                        <g transform="translate(0.245620, 0.000000)">
                          <path className="st1" d="M23.9,50C11.7,50,2.2,41.9,0.1,30.1C0.1,30,0,29.8,0,29.5l12,0c0.1,0.2,0.1,0.3,0.1,0.4c1.8,5.5,6.6,8.4,11.8,8.4c5.2,0,10-2.8,11.8-8.4c0-0.1,0.1-0.2,0.1-0.4h12c0,0.2-0.1,0.4-0.1,0.6C45.5,41.8,36,50,23.9,50z M23.9,0c12.7,0,22.5,8.8,24,21.5c0,0.2,0.1,0.5,0.1,0.8H36.1c0-0.2-0.1-0.4-0.1-0.5c-1.3-6.7-6.5-10.1-12.2-10.1c-5.6,0-10.8,3.3-12.2,9.9c0,0.2-0.1,0.4-0.1,0.7H-0.2c0.1-0.5,0.1-0.9,0.1-1.2C1.5,8.6,11.3,0,23.9,0z"/>
                        </g>
                      </g>
                    </g>
                  </g>
                </g>
              </g>
            </svg>
          </div>
        </div>

        {/* Orbit 2 - Outer circle (4 logos) */}
        <div className="orbit orbit-2">
          <div className="logo logo-7">
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 395.1 86">
              <style type="text/css">
                {`.st0{fill:#333333;} .st1{fill:#FCB400;} .st2{fill:#18BFFF;} .st3{fill:#F82B60;} .st4{fill:#BA1E45;}`}
              </style>
              <g>
                <path className="st0" d="M158.5,49.5L151.7,31c-0.3-0.8-1.4-0.8-1.6,0l-6.9,18.5c-0.2,0.6,0.2,1.2,0.8,1.2h13.8C158.3,50.7,158.8,50.1,158.5,49.5 M161.7,59.9H140c-0.4,0-0.7,0.2-0.8,0.6L134.9,72c-0.1,0.3-0.5,0.6-0.8,0.6h-9.4c-0.6,0-1-0.6-0.8-1.2l21.7-54.1c0.1-0.3,0.5-0.5,0.8-0.5h9c0.4,0,0.7,0.2,0.8,0.5l21.7,54.1c0.2,0.6-0.2,1.2-0.8,1.2h-9.4c-0.4,0-0.7-0.2-0.8-0.6l-4.3-11.5C162.4,60.2,162.1,59.9,161.7,59.9 M182.3,35.8h8.1c0.5,0,0.9,0.4,0.9,0.9v35.1c0,0.5-0.4,0.9-0.9,0.9h-8.1c-0.5,0-0.9-0.4-0.9-0.9V36.6C181.4,36.2,181.8,35.8,182.3,35.8 M220.6,44.3c0,0.5-0.4,0.9-0.9,0.9h-0.2c-4,0-6.9,1-8.8,2.9c-1.9,1.9-2.8,5.1-2.8,9.5v14.2c0,0.5-0.4,0.9-0.9,0.9h-8c-0.5,0-0.9-0.4-0.9-0.9V36.6c0-0.5,0.4-0.9,0.9-0.9h7.9c0.5,0,0.9,0.4,0.9,0.9v6.9h0.2c1-2.9,2.5-5.1,4.5-6.6c2.1-1.5,4.6-2.3,7.7-2.3h0.4V44.3z M242.7,44c-0.5,0-0.9,0.4-0.9,0.9v14.6c0,1.5,0.3,2.6,0.9,3.2c0.6,0.6,1.6,1,3,1h0.8c0.5,0,0.9,0.4,0.9,0.9v7.3c0,0.5-0.4,0.9-0.9,0.9h-3.4c-3.6,0-6.3-0.9-8.2-2.7c-1.9-1.8-2.9-4.6-2.9-8.2V44.9c0-0.5-0.4-0.9-0.9-0.9h-5c-0.5,0-0.9-0.4-0.9-0.9v-6.5c0-0.5,0.4-0.9,0.9-0.9h5c0.5,0,0.9-0.4,0.9-0.9v-13c0-0.5,0.4-0.9,0.9-0.9h8.1c0.5,0,0.9,0.4,0.9,0.9v13c0,0.5,0.4,0.9,0.9,0.9h5.8c0.5,0,0.9,0.4,0.9,0.9v6.5c0,0.5-0.4,0.9-0.9,0.9H242.7z M279.6,61.7c1.8-1.9,2.7-4.4,2.7-7.5c0-3.1-0.9-5.6-2.7-7.5c-1.8-1.9-4.2-2.9-7.2-2.9c-3,0-5.4,1-7.2,2.9c-1.8,1.9-2.7,4.4-2.7,7.5c0,3.1,0.9,5.6,2.7,7.5c1.8,1.9,4.2,2.9,7.2,2.9C275.3,64.6,277.7,63.6,279.6,61.7 M261,71.5c-2.7-1.5-4.8-3.8-6.4-6.7c-1.6-2.9-2.4-6.5-2.4-10.6c0-4.1,0.8-7.6,2.4-10.6c1.6-2.9,3.7-5.2,6.4-6.7c2.7-1.5,5.7-2.3,8.9-2.3c2.9,0,5.3,0.6,7.4,1.7c2,1.1,3.7,2.7,4.9,4.7h0.2v-4.3c0-0.5,0.4-0.9,0.9-0.9h8c0.5,0,0.9,0.4,0.9,0.9v35.1c0,0.5-0.4,0.9-0.9,0.9h-8c-0.5,0-0.9-0.4-0.9-0.9v-4.3h-0.2c-1.2,2-2.9,3.6-4.9,4.7c-2,1.1-4.5,1.7-7.4,1.7C266.6,73.8,263.7,73,261,71.5 M325.2,61.7c1.8-1.9,2.7-4.4,2.7-7.5c0-3.1-0.9-5.6-2.7-7.5c-1.8-1.9-4.2-2.9-7.2-2.9c-3,0-5.4,1-7.2,2.9c-1.8,1.9-2.8,4.4-2.8,7.5c0,3.1,0.9,5.6,2.8,7.5c1.8,1.9,4.2,2.9,7.2,2.9C321,64.6,323.4,63.6,325.2,61.7 M313.1,72.1c-2-1.1-3.7-2.7-4.9-4.7h-0.2v4.3c0,0.5-0.4,0.9-0.9,0.9h-8.1c-0.5,0-0.9-0.4-0.9-0.9V17.7c0-0.5,0.4-0.9,0.9-0.9h8.1c0.5,0,0.9,0.4,0.9,0.9V41h0.2c1.2-2,2.9-3.6,4.9-4.7c2-1.1,4.5-1.7,7.4-1.7c3.2,0,6.2,0.8,8.9,2.3c2.7,1.5,4.8,3.8,6.4,6.7c1.6,2.9,2.4,6.5,2.4,10.6c0,4.1-0.8,7.6-2.4,10.6c-1.6,2.9-3.7,5.2-6.4,6.7c-2.7,1.5-5.7,2.3-8.9,2.3C317.6,73.8,315.2,73.2,313.1,72.1 M351.8,72.6h-8.1c-0.5,0-0.9-0.4-0.9-0.9V17.7c0-0.5,0.4-0.9,0.9-0.9h8.1c0.5,0,0.9,0.4,0.9,0.9v54.1C352.6,72.2,352.3,72.6,351.8,72.6 M370.8,44.3c-1.3,1-2.2,2.5-2.7,4.5c-0.1,0.5,0.3,1.1,0.9,1.1h15.3c0.5,0,1-0.5,0.9-1c-0.3-1.9-1.1-3.3-2.4-4.5c-1.5-1.3-3.5-2-5.9-2C374.4,42.5,372.4,43.1,370.8,44.3 M390.4,39.7c3.1,3.4,4.7,8.3,4.7,14.5v1c0,0.5-0.4,0.9-0.9,0.9h-25.6c-0.5,0-1,0.5-0.9,1c0.4,2.3,1.4,4.2,3.1,5.5c1.9,1.6,4.3,2.4,7.1,2.4c3.7,0,7.1-1.4,10.3-4.3c0.4-0.4,1-0.3,1.3,0.2l3.9,5.6c0.3,0.4,0.2,0.9-0.1,1.2c-1.9,1.7-4,3.1-6.4,4.2c-2.6,1.2-5.6,1.9-9,1.9c-4,0-7.5-0.8-10.4-2.4c-3-1.6-5.3-3.9-6.9-6.9c-1.6-2.9-2.5-6.4-2.5-10.2c0-3.9,0.8-7.3,2.4-10.3c1.6-3,3.8-5.3,6.7-6.9c2.9-1.6,6.2-2.5,10-2.5C382.9,34.6,387.3,36.3,390.4,39.7 M192.3,22.8c0,3.3-2.7,6-6,6s-6-2.7-6-6s2.7-6,6-6S192.3,19.5,192.3,22.8"/>
                <path className="st1" d="M45.9,1.1L7.6,16.9c-2.1,0.9-2.1,3.9,0,4.8L46.1,37c3.4,1.3,7.1,1.3,10.5,0l38.5-15.3c2.1-0.9,2.2-3.9,0-4.8L56.8,1.1C53.3-0.4,49.4-0.4,45.9,1.1"/>
                <path className="st2" d="M54.8,45.3v38.1c0,1.8,1.8,3.1,3.5,2.4l42.9-16.6c1-0.4,1.6-1.3,1.6-2.4V28.7c0-1.8-1.8-3.1-3.5-2.4L56.4,42.9C55.4,43.3,54.8,44.2,54.8,45.3"/>
                <path className="st3" d="M0,28.8v35.8c0,1.9,2.2,3.1,3.9,2.3L30.7,54l1.3-0.6L0.8,27.1C0.4,27.5,0,28.1,0,28.8z"/>
                <path className="st4" d="M44.6,42.6L3.8,26.4c-0.8-0.3-1.7-0.2-2.3,0.2c-0.2,0.1-0.5,0.3-0.7,0.5L32,53.4l12.7-6.1C46.8,46.3,46.6,43.4,44.6,42.6z"/>
              </g>
            </svg>
          </div>
          <div className="logo logo-8">
            <div className="text-xs font-medium">HubSpot</div>
          </div>
          <div className="logo logo-9">
            <div className="text-xs font-medium">Zapier</div>
          </div>
          <div className="logo logo-10">
            <div className="text-xs font-medium">Make</div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ToolsRadarAnimation;